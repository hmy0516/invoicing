...
public interface PurchorderMapper{
    ...
    @Insert("insert into t_lsc_purchorder(i_pono,c_uuid,dt_dt,dt_begin,dt_end,c_clerk,c_vendor,c_base,c_sale,c_check,dt_check,c_checkout,dt_checkout,c_invalid,dt_invalid,c_comment,i_bno,i_status values (#{i_pono},#{c_uuid},#{dt_dt},#{dt_begin},#{dt_end},#{c_clerk},#{c_vendor},#{c_base},#{c_sale},#{c_check},#{dt_check},#{c_checkout},#{dt_checkout},#{c_invalid},#{dt_invalid},#{c_comment},#{i_bno},#{i_status})")
